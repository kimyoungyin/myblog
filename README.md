...
- **소셜 기능**
    - 댓글/대댓글, 좋아요, 조회수 집계
    - 좋아요/댓글은 낙관적 업데이트를 적용해 즉각적인 UI 반응 제공
...
- **React Query 기반 무한 스크롤 & 캐싱**
    - 글 목록/검색 페이지에서 서버에서 `prefetchInfiniteQuery` + `dehydrate` 후,
      클라이언트에서 `useInfiniteQuery`로 이어받아 **SSR 초기 데이터 + 무한 스크롤 캐싱**을 구현했습니다.
